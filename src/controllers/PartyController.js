...
const getAllDeletedParty = async (req, res, next) => {
  try {
    ...
    let query = [{ isDelete: deletedParty }, { userId: _id }];
    ...
    let party = await model.Party.aggregate([
      ...
      },
      {
        $replaceRoot: {
          newRoot: {
            $mergeObjects: [{ cuttingType: "$data.cuttingType" }, "$$ROOT"],
          },
        },
      },
      {
  ...
  try {
    ...
    if (updateData["cuttingType"]) {
      updateData["cuttingType"].map((cuttingData) => {
        ...
        if (cutID !== "") {
          let getIndex = partyData.cuttingType.findIndex(
            (d) => d._id.toString() == cutID
          );
          ...
          let typeExist = partyData.cuttingType.find(
            (d) => d.cutType === cuttingData.cutType
          );
          ...
          if (!typeExist) {
            ...
            throw {
              message: `${typeExist.cutType} Cutting Type is Already Exists`,
            };
          }
        }
      });
      ...
      if (oldCutType && oldCutType.cutType) {
        await model.Loat.updateMany(
          {
            cuttingType: oldCutType.cutType,
            cutId: oldCutType._id,
            partyId: updateData.partyId,
          },
          { $set: { cuttingType: newCutType.cutType } }
        );
      }

      updateData["cuttingType"] = [
        ...cuttingTypeData,
        ...partyData.cuttingType,
      ];
    }
...
const getPartyLoatDateWise = async (req, res, next) => {
  try {
            ...
            $push: {
              ...
              price: {
                $cond: {
                  ...
                  then: {
                    $round: [{ $multiply: ["$numOfDimonds", "$loatPrice"] }, 2],
                  },
                  else: {
                    $round: [{ $multiply: ["$loatWeight", "$loatPrice"] }, 2],
                  },
                },
              },
            },
    ...
    let party = await model.Party.aggregate([
      {
        $match: {
          $and: [{ _id: new ObjectId(partyId) }, { userId: _id }],
        },
      },
      {
        $project: {
      ...
      for (let i = 0; i < totalLoatLength; i++) {
        ...
        if (totalTypeWiseLoatLength > 0) {
          for (let j = 0; j < totalTypeWiseLoatLength; j++) {
            const index1 = allKeys.paymentDetails.findIndex(
              (d) => d.key === `Total Diamonds (${tLoat[j]._id.type})`
            );
            const index2 = allKeys.paymentDetails.findIndex(
              (d) => d.key === `Total Weight (${tLoat[j]._id.type})`
            );
            const index3 = allKeys.paymentDetails.findIndex(
              (d) => d.key === `Total Amount (${tLoat[j]._id.type})`
            );
            const index4 = allKeys.paymentDetails.findIndex(
              (d) => d.key === `Diamond Wise Count (${tLoat[j]._id.type})`
            );
            const index5 = allKeys.paymentDetails.findIndex(
              (d) => d.key === `Diamond Wise Weight (${tLoat[j]._id.type})`
            );
            const index6 = allKeys.paymentDetails.findIndex(
              (d) => d.key === `Diamond Wise Amount (${tLoat[j]._id.type})`
            );
            const index7 = allKeys.paymentDetails.findIndex(
              (d) => d.key === `Weight Wise Diamond (${tLoat[j]._id.type})`
            );
            const index8 = allKeys.paymentDetails.findIndex(
              (d) => d.key === `Weight Wise Weight (${tLoat[j]._id.type})`
            );
            const index9 = allKeys.paymentDetails.findIndex(
              (d) => d.key === `Weight Wise Amount (${tLoat[j]._id.type})`
            );
            ...
            if ((index4 !== -1 || index7 !== -1) && loatsLength > 0) {
              for (let k = 0; k < loatsLength; k++) {
                if (allLoats[k].multiWithDiamonds) {
                  if (index4 !== -1) {
                    allKeys.paymentDetails[index1].value +=
                      allLoats[k].numOfDimonds;
                    allKeys.paymentDetails[index2].value +=
                      allLoats[k].loatWeight;
                    allKeys.paymentDetails[index3].value += allLoats[k].price;

                    allKeys.paymentDetails[index4].value +=
                      allLoats[k].numOfDimonds;
                    allKeys.paymentDetails[index5].value +=
                      allLoats[k].loatWeight;
                    ...
                    newAllKeys.paymentDetails[index1].value +=
                      allLoats[k].numOfDimonds;
                    newAllKeys.paymentDetails[index2].value +=
                      allLoats[k].loatWeight;
                    newAllKeys.paymentDetails[index3].value +=
                      allLoats[k].price;

                    newAllKeys.paymentDetails[index4].value +=
                      allLoats[k].numOfDimonds;
                    newAllKeys.paymentDetails[index5].value +=
                      allLoats[k].loatWeight;
                    newAllKeys.paymentDetails[index6].value +=
                      allLoats[k].price;

                    newAllKeys.TotalDimonds += allLoats[k].numOfDimonds;
                    newAllKeys.TotalWeight += allLoats[k].loatWeight;
                    newAllKeys.TotalDiamondWiseCount +=
                      allLoats[k].numOfDimonds;
                  ...
                  if (index7 !== -1) {
                    ...
                    allKeys.paymentDetails[index7].value +=
                      allLoats[k].numOfDimonds;
                    allKeys.paymentDetails[index8].value +=
                      allLoats[k].loatWeight;
    ...
    let loats = await model.Loat.aggregate([
      ...
      },
      {
        $addFields: {
          ...
          totalWeightWisePayment: {
            $round: [{ $sum: "$loats.weightWiseprice" }, 2],
          },
          totalDimondsWisePayment: {
            $round: [{ $sum: "$loats.diamondWiseprice" }, 2],
          },
        },
      },
      {
        $addFields: {
          totalPayment: {
            $round: [
              { $sum: ["$totalWeightWisePayment", "$totalDimondsWisePayment"] },
              2,
            ],
          },
        },
          ...
          for (let month = 0; month < totalMonthLength; month++) {
            ...
            if (dateWiseLoats && totalLoatLength > 0) {
              for (let date = 0; date < totalLoatLength; date++) {
                const totalTypeWiseLoatLength =
                  dateWiseLoats[date].typeWiseLoat.length;
                ...
                if (totalTypeWiseLoatLength > 0) {
                  for (let j = 0; j < totalTypeWiseLoatLength; j++) {
                    const index1 = dayWiseTotal.paymentDetails.findIndex(
                      (d) => d.key === `Total Diamonds (${tLoat[j]._id.type})`
                    );
                    const index2 = dayWiseTotal.paymentDetails.findIndex(
                      (d) => d.key === `Total Weight (${tLoat[j]._id.type})`
                    );
                    const index3 = dayWiseTotal.paymentDetails.findIndex(
                      (d) => d.key === `Total Amount (${tLoat[j]._id.type})`
                    );
                    const index4 = dayWiseTotal.paymentDetails.findIndex(
                      (d) =>
                        d.key === `Diamond Wise Count (${tLoat[j]._id.type})`
                    );
                    const index5 = dayWiseTotal.paymentDetails.findIndex(
                      (d) =>
                        d.key === `Diamond Wise Weight (${tLoat[j]._id.type})`
                    );
                    const index6 = dayWiseTotal.paymentDetails.findIndex(
                      (d) =>
                        d.key === `Diamond Wise Amount (${tLoat[j]._id.type})`
                    );
                    const index7 = dayWiseTotal.paymentDetails.findIndex(
                      (d) =>
                        d.key === `Weight Wise Diamond (${tLoat[j]._id.type})`
                    );
                    const index8 = dayWiseTotal.paymentDetails.findIndex(
                      (d) =>
                        d.key === `Weight Wise Weight (${tLoat[j]._id.type})`
                    );
                    const index9 = dayWiseTotal.paymentDetails.findIndex(
                      (d) =>
                        d.key === `Weight Wise Amount (${tLoat[j]._id.type})`
                    );
                    const loatsLength = tLoat[j].loats.length;
                    const allLoats = tLoat[j].loats;
                    ...
                    if ((index4 !== -1 || index7 !== -1) && loatsLength > 0) {
                      for (let k = 0; k < loatsLength; k++) {
                        if (allLoats[k].multiWithDiamonds) {
                          if (index4 !== -1) {
                            allTotal.paymentDetails[index1].value +=
                              allLoats[k].numOfDimonds;
                            allTotal.paymentDetails[index2].value +=
                              allLoats[k].loatWeight;
                            allTotal.paymentDetails[index3].value +=
                              allLoats[k].price;

                            allTotal.paymentDetails[index4].value +=
                              allLoats[k].numOfDimonds;
                            allTotal.paymentDetails[index5].value +=
                              allLoats[k].loatWeight;
                            allTotal.paymentDetails[index6].value +=
                              allLoats[k].price;

                            allTotal.TotalDimonds += allLoats[k].numOfDimonds;
                            allTotal.TotalWeight += allLoats[k].loatWeight;
                            allTotal.TotalDiamondWiseCount +=
                              allLoats[k].numOfDimonds;
                            allTotal.TotalDiamondWiseWeight +=
                              allLoats[k].loatWeight;
                            allTotal.TotalDiamondWiseAmount +=
                              allLoats[k].price;
                            allTotal.TotalAmount += allLoats[k].price;

                            yearWiseTotal.paymentDetails[index1].value +=
                              allLoats[k].numOfDimonds;
                            yearWiseTotal.paymentDetails[index2].value +=
                              allLoats[k].loatWeight;
                            yearWiseTotal.paymentDetails[index3].value +=
                              allLoats[k].price;

                            yearWiseTotal.paymentDetails[index4].value +=
                              allLoats[k].numOfDimonds;
                            yearWiseTotal.paymentDetails[index5].value +=
                              allLoats[k].loatWeight;
                            yearWiseTotal.paymentDetails[index6].value +=
                              allLoats[k].price;

                            yearWiseTotal.TotalDimonds +=
                              allLoats[k].numOfDimonds;
                            yearWiseTotal.TotalWeight += allLoats[k].loatWeight;
                            yearWiseTotal.TotalDiamondWiseCount +=
                              allLoats[k].numOfDimonds;
                            yearWiseTotal.TotalDiamondWiseWeight +=
                              allLoats[k].loatWeight;
                            yearWiseTotal.TotalDiamondWiseAmount +=
                              allLoats[k].price;
                            yearWiseTotal.TotalAmount += allLoats[k].price;

                            monthWiseTotal.paymentDetails[index1].value +=
                              allLoats[k].numOfDimonds;
                            monthWiseTotal.paymentDetails[index2].value +=
                              allLoats[k].loatWeight;
                            monthWiseTotal.paymentDetails[index3].value +=
                              allLoats[k].price;

                            monthWiseTotal.paymentDetails[index4].value +=
                              allLoats[k].numOfDimonds;
                            monthWiseTotal.paymentDetails[index5].value +=
                              allLoats[k].loatWeight;
                            monthWiseTotal.paymentDetails[index6].value +=
                              allLoats[k].price;

                            monthWiseTotal.TotalDimonds +=
                              allLoats[k].numOfDimonds;
                            monthWiseTotal.TotalWeight +=
                              allLoats[k].loatWeight;
                            monthWiseTotal.TotalDiamondWiseCount +=
                              allLoats[k].numOfDimonds;
                            monthWiseTotal.TotalDiamondWiseWeight +=
                              allLoats[k].loatWeight;
                            monthWiseTotal.TotalDiamondWiseAmount +=
                              allLoats[k].price;
                            monthWiseTotal.TotalAmount += allLoats[k].price;

                            dayWiseTotal.paymentDetails[index1].value +=
                              allLoats[k].numOfDimonds;
                            dayWiseTotal.paymentDetails[index2].value +=
                              allLoats[k].loatWeight;
                            dayWiseTotal.paymentDetails[index3].value +=
                              allLoats[k].price;

                            dayWiseTotal.paymentDetails[index4].value +=
                              allLoats[k].numOfDimonds;
                            dayWiseTotal.paymentDetails[index5].value +=
                              allLoats[k].loatWeight;
                            dayWiseTotal.paymentDetails[index6].value +=
                              allLoats[k].price;

                            dayWiseTotal.TotalDimonds +=
                              allLoats[k].numOfDimonds;
                            dayWiseTotal.TotalWeight += allLoats[k].loatWeight;
                            dayWiseTotal.TotalDiamondWiseCount +=
                              allLoats[k].numOfDimonds;
                            dayWiseTotal.TotalDiamondWiseWeight +=
                              allLoats[k].loatWeight;
                            dayWiseTotal.TotalDiamondWiseAmount +=
                              allLoats[k].price;
                            dayWiseTotal.TotalAmount += allLoats[k].price;

                            typeWiseTotal.loatsDiamondsWisePrice +=
                              allLoats[k].price;
                            typeWiseTotal.loatsCaret += allLoats[k].loatWeight;
                          }
                        } else {
                          if (index7 !== -1) {
                            ...
                            allTotal.paymentDetails[index7].value +=
                              allLoats[k].numOfDimonds;
                            allTotal.paymentDetails[index8].value +=
                              allLoats[k].loatWeight;
                            allTotal.paymentDetails[index9].value +=
                              allLoats[k].price;

                            allTotal.TotalDimonds += allLoats[k].numOfDimonds;
                            allTotal.TotalWeight += allLoats[k].loatWeight;
                            allTotal.TotalWeightWiseCount +=
                              allLoats[k].numOfDimonds;
                            allTotal.TotalWeightWiseWeight +=
                              allLoats[k].loatWeight;
                            ...
                            yearWiseTotal.paymentDetails[index7].value +=
                              allLoats[k].numOfDimonds;
                            yearWiseTotal.paymentDetails[index8].value +=
                              allLoats[k].loatWeight;
                            yearWiseTotal.paymentDetails[index9].value +=
                              allLoats[k].price;

                            yearWiseTotal.TotalDimonds +=
                              allLoats[k].numOfDimonds;
                            yearWiseTotal.TotalWeight += allLoats[k].loatWeight;
                            yearWiseTotal.TotalWeightWiseCount +=
                              allLoats[k].numOfDimonds;
                            yearWiseTotal.TotalWeightWiseWeight +=
                              allLoats[k].loatWeight;
                            yearWiseTotal.TotalWeightWiseAmount +=
                              allLoats[k].price;
                            yearWiseTotal.TotalAmount += allLoats[k].price;
                            ...
                            monthWiseTotal.paymentDetails[index7].value +=
                              allLoats[k].numOfDimonds;
                            monthWiseTotal.paymentDetails[index8].value +=
                              allLoats[k].loatWeight;
                            monthWiseTotal.paymentDetails[index9].value +=
                              allLoats[k].price;
                            ...
                            monthWiseTotal.TotalWeight +=
                              allLoats[k].loatWeight;
                            monthWiseTotal.TotalWeightWiseCount +=
                              allLoats[k].numOfDimonds;
                            monthWiseTotal.TotalWeightWiseWeight +=
                              allLoats[k].loatWeight;
                            monthWiseTotal.TotalWeightWiseAmount +=
                              allLoats[k].price;
                            monthWiseTotal.TotalAmount += allLoats[k].price;
                            ...
                            dayWiseTotal.paymentDetails[index7].value +=
                              allLoats[k].numOfDimonds;
                            dayWiseTotal.paymentDetails[index8].value +=
                              allLoats[k].loatWeight;
                            dayWiseTotal.paymentDetails[index9].value +=
                              allLoats[k].price;

                            dayWiseTotal.TotalDimonds +=
                              allLoats[k].numOfDimonds;
                            dayWiseTotal.TotalWeight += allLoats[k].loatWeight;
                            dayWiseTotal.TotalWeightWiseCount +=
                              allLoats[k].numOfDimonds;
                            dayWiseTotal.TotalWeightWiseWeight +=
                              allLoats[k].loatWeight;
                            dayWiseTotal.TotalWeightWiseAmount +=
                              allLoats[k].price;
                            dayWiseTotal.TotalAmount += allLoats[k].price;
            ...
            if (dateWiseLoats && totalLoatLength > 0) {
              for (let date = 0; date < totalLoatLength; date++) {
                ...
                if (getParty[0]) {
                  ...
                  const exist = partyDetails.findIndex(
                    (d) => d._id.toString() === cloneParty._id.toString()
                  );
                ...
                if (typeWiseLoat && totalLoatEntryLength > 0) {
                  for (
                    ...
                    const existParty = partyDetails.findIndex(
                      (d) =>
                        d._id.toString() ===
                        dateWiseLoats[date]._id.party.toString()
                    );
                    if (existParty !== -1) {
                      if (
                        !partyDetails[existParty].loatHaveMonth.includes(
                          loatMonth
                        )
                      ) {
                      ...
                      if (
                        !partyDetails[existParty].loatHaveYear.includes(
                          loatYear
                        )
                      ) {
                      ...
                      const existYear = partyDetails[
                        existParty
                      ].payment.findIndex((d) => d.loatYear === loatYear);
                      let existMonth = -1;
                      if (existYear !== -1) {
                        existMonth = partyDetails[existParty].payment[
                          existYear
                        ].details.findIndex((d) => d.loatMonth === loatMonth);
                      }

                      if (existMonth !== -1 && existYear !== -1) {
                        const findLoatsIndex = partyDetails[existParty].payment[
                          existYear
                        ].details[existMonth].loat.findIndex(
                          (d) => d.type === typeWiseLoat[typeLoat].loats[0].type
                        );
                        if (findLoatsIndex !== -1) {
                          partyDetails[existParty].payment[existYear].details[
                            existMonth
                          ].loat[findLoatsIndex].loats.push(
                            ...typeWiseLoat[typeLoat].loats
                          );
                        } else {
                          partyDetails[existParty].payment[existYear].details[
                            existMonth
                          ].loat.push({
                            ...{ type: typeWiseLoat[typeLoat].loats[0].type },
                            ...{ loats: typeWiseLoat[typeLoat].loats },
                          });
                        }
                      } else {
                        if (existYear !== -1) {
                          if (existMonth === -1) {
                            partyDetails[existParty].payment[
                              existYear
                            ].details.push({
                              loatMonth,
                              loat: [
                          ...
                          partyDetails[existParty].payment.push({
                            loatYear,
                            details: [
                              {
                                loatMonth,
                                loat: [
                                  {
                                    ...{
                                      type: typeWiseLoat[typeLoat].loats[0]
                                        .type,
                                    },
                                    ...{ loats: typeWiseLoat[typeLoat].loats },
                                  },
              ...
              for (let month = 0; month < totalMonthLength; month++) {
                ...
                if (totalLoatLength > 0) {
                  for (let j = 0; j < totalLoatLength; j++) {
                    const index1 = monthWiseTotal.paymentDetails.findIndex(
                      (d) => d.key === `Total Diamonds (${tLoat[j].type})`
                    );
                    const index2 = monthWiseTotal.paymentDetails.findIndex(
                      (d) => d.key === `Total Weight (${tLoat[j].type})`
                    );
                    const index3 = monthWiseTotal.paymentDetails.findIndex(
                      (d) => d.key === `Total Amount (${tLoat[j].type})`
                    );
                    const index4 = monthWiseTotal.paymentDetails.findIndex(
                      (d) => d.key === `Diamond Wise Count (${tLoat[j].type})`
                    );
                    const index5 = monthWiseTotal.paymentDetails.findIndex(
                      (d) => d.key === `Diamond Wise Weight (${tLoat[j].type})`
                    );
                    const index6 = monthWiseTotal.paymentDetails.findIndex(
                      (d) => d.key === `Diamond Wise Amount (${tLoat[j].type})`
                    );
                    const index7 = monthWiseTotal.paymentDetails.findIndex(
                      (d) => d.key === `Weight Wise Diamond (${tLoat[j].type})`
                    );
                    const index8 = monthWiseTotal.paymentDetails.findIndex(
                      (d) => d.key === `Weight Wise Weight (${tLoat[j].type})`
                    );
                    const index9 = monthWiseTotal.paymentDetails.findIndex(
                      (d) => d.key === `Weight Wise Amount (${tLoat[j].type})`
                    );
        ...
        if (yearWiseLoats && totalMonthLength > 0) {
          for (let month = 0; month < totalMonthLength; month++) {
            ...
            partyDetails.forEach((party) => {
              ...
              if (
                party.loatHaveMonth.includes(yearWiseLoats[month]._id.month)
              ) {
                party.payment.forEach((pay) => {
                  if (party.loatHaveYear.includes(pay.loatYear)) {
                    const yearIndex = newPartyDetails.findIndex(
                      (d) => d.year === pay.loatYear
                    );
                    ...
                    if (yearIndex === -1) {
                      if (!searchMonth && !searchYear) {
                        ...
                        const totalPrice = {
                          totalPrices:
                            pay.details[0].monthWiseTotal.TotalAmount || 0,
                          totalDimonds:
                            pay.details[0].monthWiseTotal.TotalDimonds || 0,
                          totalWeight:
                            pay.details[0].monthWiseTotal.TotalWeight || 0,
                        };
                        ...
                        newPartyDetails.push({
                          ...
                          ...{
                            details: [
                              {
                                month: pay.details[0].loatMonth,
                                ...{
                                  details: [
                                    {
                                      ...partyDetails,
                                      monthWiseTotal:
                                        pay.details[0].monthWiseTotal,
                                    },
                                  ],
                                },
                              },
                      ...
                      const oldMonthIndex = pay.details.findIndex(
                        (d) => d.loatMonth === yearWiseLoats[month]._id.month
                      );
                      const monthIndex = newPartyDetails[
                        yearIndex
                      ].details.findIndex(
                        (d) => d.month === yearWiseLoats[month]._id.month
                      );

                      if (monthIndex === -1) {
                        if (oldMonthIndex !== -1) {
                          newPartyDetails[yearIndex].totalPrices +=
                            pay.details[
                              oldMonthIndex
                            ].monthWiseTotal.TotalAmount;
                          newPartyDetails[yearIndex].totalDimonds +=
                            pay.details[
                              oldMonthIndex
                            ].monthWiseTotal.TotalDimonds;
                          newPartyDetails[yearIndex].totalWeight +=
                            pay.details[
                              oldMonthIndex
                            ].monthWiseTotal.TotalWeight;
                          newPartyDetails[yearIndex].details.push({
                            month: pay.details[oldMonthIndex].loatMonth,
                            ...{
                              details: [
                                {
                                  ...partyDetails,
                                  monthWiseTotal:
                                    pay.details[oldMonthIndex].monthWiseTotal,
                                },
                              ],
                            },
                          });
                        ...
                        const partyExists = newPartyDetails[yearIndex].details[
                          monthIndex
                        ].details.findIndex((d) => d.partyId === party._id);
                        if (oldMonthIndex !== -1 && partyExists === -1) {
                          ...
                          newPartyDetails[yearIndex].details[
                            monthIndex
                          ].details.push({
                            ...partyDetails,
                            monthWiseTotal:
...
const generateAllInvoicePDF = async (req, res, next) => {
  try {
    let { allInvoiceData, date, user, totalDiamond, totalWeight, totalPrice } =
      req.body;
      ...
      if (err) {
        ...
        let options = {
          ...
          footer: {
            ...
            contents: {
              default:
                "<span style='color: #444;text-align: center; padding-top: 7px'>{{page}}</span>/<span>{{pages}} pages</span>", // fallback values
            },
          },
        };
        pdf
          .create(data, options)
          .toFile(`./uploads/${fileName}`, async (err, data) => {
            if (err) {
              res.send(errorRes(err));
            } else {
              const uploadFile = await helper.updloadFileToFirebase(
